...
namespace Bronk
{
    class Pathfinding
    {
        ...
        public class Node
        {
			...
			public bool isBlocked(float time) {
				return cube.TypeTimeline.GetValue(time) != GameWorld.BlockType.DirtGround;
            }
        }
        ...
        public List<Node> findPath(Vector3 start, Vector3 end)
        {
			...
			if (startNode.isBlocked(Time.time) || endNode.isBlocked(Time.time))
            {
        ...
        public void printGraph(List<Node> targetNodes)
        {
            ...
            if (targetNodes == null)
            {
                for (var y = 0; y < _sizeY; ++y)
                {
                    ...
                    {
                        var c = _nodes[x, y];
						sb.Append(c.isBlocked(Time.time) ? "x" : "-");
                    }
